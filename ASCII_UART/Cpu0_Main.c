...
void core0_main(void)
{
    ...
    while(1)
    {
        ...
        {
            count++;
            sprintf(data,"\n\r Counter Value is %d ",count);
            send_data(data, strlen(data)); /* Acknowledgment via UART - Triggered by DMA transaction done */
            conv[0]=(count)+'0';
            conv[1]='\0';
            send_data(conv, sizeof(conv)); /* Acknowledgment via UART - Triggered by DMA transaction done */
        }
        count=0;
    }
}
